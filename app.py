...
from database import db, init_app, User, Ticket, Organization, FAQ, Partner, EscalationLevel, SLARule, Category, Message, CommonQuery, Feedback, Attachment, FAQCategory, Workflow, WorkflowStep
...
app.config['BABEL_SUPPORTED_LOCALES'] = [
    # All working languages with English first
    'en', 'es', 'ar', 'hi', 'it', 'ja', 'ko', 'pt', 'ru', 'ur', 'zh'
]
...
try:
    if hasattr(config_obj, 'ODOO_URL') and config_obj.ODOO_URL:
        ...
        logger.warning(
            "Odoo configuration not found - Odoo features will be disabled")
...
def get_available_languages():
    ...
    working_languages = ['en', 'ar', 'es', 'hi',
                         'it', 'ja', 'ko', 'pt', 'ru', 'ur', 'zh']
    ...
    if os.path.exists(translations_dir):
        for lang_code in working_languages:
            mo_file = os.path.join(
                translations_dir, lang_code, 'LC_MESSAGES', 'messages.mo')
...
def setup_database():
    ...
    try:
        ...
        logger.info(
            f"Database URI configured for MSSQL: {config_obj.SQLALCHEMY_DATABASE_URI}")
        return True
    except Exception as e:
...
# Initialize database and import database
db = init_app(app)

# Database models are now imported from database.py to avoid circular imports
...
def handle_send_message(data):
    try:
        ...
        sender_id = getattr(current_user, 'UserID',
                            None) if current_user.is_authenticated else None

        logger.info(
            f"📨 Received message for ticket #{ticket_id}: {content[:50]}... (is_admin: {is_admin})")
...
def logout():
    ...
    logout_user()
    flash(
        f'Goodbye {user_name}! You have been logged out successfully.', 'success')
...
def edit_profile():
    ...
    if request.method == 'POST':
        try:
            ...
            current_user.Position = request.form.get(
                'position', current_user.Position)
            current_user.Department = request.form.get(
                'department', current_user.Department)
            current_user.Phone = request.form.get('phone', current_user.Phone)
            current_user.PreferredLanguage = request.form.get(
                'language', current_user.PreferredLanguage)
...
def my_tickets():
    ...
    try:
        ...
        user_tickets = Ticket.query.filter_by(
            UserID=current_user.UserID).order_by(Ticket.CreatedAt.desc()).all()
...
def create_ticket():
    try:
        ...
        if current_user.is_authenticated:
            ...
            user = current_user
            logger.info(
                f"Using authenticated user: {user.Name} from {user.OrganizationName}")
        else:
            ...
            if data.get('name') or data.get('email'):
                try:
                    ...
                    logger.info(
                        f"Created/found guest user with ID: {user.UserID}")
                except Exception as e:
        ...
        ticket_priority = data.get(
            'priority', user.PriorityLevel if user else 'medium')
        ...
        logger.info(
            f"Auto-assigned escalation level: {escalation_level} (based on priority: {ticket_priority})")

        # Auto-detect country from user's location
        ticket_country = 'Unknown'
        try:
            location_info = location_service.detect_country_from_request(
                request)
            if location_info and location_info.get('country'):
                ticket_country = location_info['country']
                logger.info(
                    f"Auto-detected country for new ticket: {ticket_country}")
            else:
        ...
        try:
            ...
            ticket = Ticket(
                ...
                Status='open',
                OrganizationName=user.OrganizationName if user else data.get(
            ...
            db.session.flush()  # Get the ID without committing
            logger.info(
                f"Created ticket with ID: {ticket.TicketID}, Priority: {ticket_priority}, SLA Target: {sla_target}, Organization: {ticket.OrganizationName}")
        except Exception as e:
        ...
        bot_attempted = False
        try:
            bot_response = bot_service.process_query(
                data['message'], ticket.TicketID)
            bot_attempted = True
            ...
            if bot_response and bot_response.get('confidence', 0) >= 0.7:
                ...
                logger.info(
                    f"Bot provided confident response for ticket {ticket.TicketID}")
            else:
                ...
                logger.info(
                    f"Bot response not confident enough, escalating ticket {ticket.TicketID} to human")
        ...
        try:
            if odoo_service:
                logger.info(
                    f"Syncing ticket {ticket.TicketID} to Odoo Online...")
                ...
                if user and user.Email:
                    try:
                        ...
                        logger.info(
                            f"Created/found customer in Odoo with ID: {odoo_customer_id}")
                    except Exception as e:
                        logger.warning(
                            f"Could not create customer in Odoo: {e}")
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Ticket {ticket.TicketID} synced to Odoo ticket {odoo_ticket_id}")
        ...
        try:
            # Enhanced SLA monitor will create initial SLA log automatically
            # when it processes the ticket for the first time
            logger.debug(
                f"Initial SLA will be set by enhanced monitor for ticket {ticket.TicketID}")
        except Exception as e:
        ...
        try:
            ...
            device_info = DeviceInfo(
                user_agent_string=user_agent, ip_address=ip_address)
            ...
            if device_data:
                ...
                ticket.operating_system = os_info.get(
                    'family') if os_info else None
                ticket.browser = browser_info.get(
                    'family') if browser_info else None
                ticket.browser_version = browser_info.get(
                    'version_string') if browser_info else None
                ticket.os_version = os_info.get(
                    'version_string') if os_info else None
                ...
                logger.info(
                    f"Device info captured for ticket {ticket.TicketID}: {device_data.get('device_type')} / {browser_info.get('family') if browser_info else 'Unknown'} / {os_info.get('family') if os_info else 'Unknown'}")
            else:
                logger.warning(
                    f"No device data available for ticket {ticket.TicketID}")
        except Exception as e:
            logger.error(
                f"Device tracking failed for ticket {ticket.TicketID}: {e}")
        ...
        logger.info(
            f"Successfully created ticket {ticket.TicketID} for organization {ticket.OrganizationName}")
...
def handle_messages(ticket_id):
    if request.method == 'GET':
        messages = Message.query.filter_by(
            TicketID=ticket_id).order_by(Message.CreatedAt).all()
        message_list = []

        for m in messages:
            # Get attachments for this message
            attachments = Attachment.query.filter_by(
                MessageID=m.MessageID).all()
        ...
        if not user:
            ...
            try:
                ...
                if location_info and location_info.get('country'):
                    ...
                    logger.info(
                        f"Auto-detected country for new user: {user_country}")
            except Exception as e:
...
def get_dashboard_stats():
    ...
    try:
        ...
        active_chats = Ticket.query.filter(
            Ticket.Status.in_(['open', 'in_progress'])).count()
        ...
        for ticket in sample_tickets:
            logger.info(
                f"Ticket {ticket.TicketID}: {ticket.Subject} - Status: {ticket.Status}")
        ...
        if hasattr(request, 'uv_admin'):
            ...
            logger.info(
                f"Urban Vyapari dashboard access by: {request.uv_admin['admin_name']}")
    ...
    try:
        ...
        try:
            ...
            filters = {'status': 'all', 'priority': 'all',
                       'category': 'all', 'limit': 50, 'offset': 0}
        ...
        try:
            ...
            logger.error(
                f"Database connection failed in tickets: {str(db_error)}")
            return jsonify({
                "error": "Database connection failed",
                "details": str(db_error)
            }), 500
        ...
        try:
            ...
            for ticket, user, category in tickets:
                ...
                logger.info(
                    f"Processed ticket {ticket.TicketID}: {ticket.Subject} - Priority: {ticket.Priority}, Org: {ticket_data['organization']}")

            logger.info(
                f"Returning {len(result)} tickets to admin panel sorted by priority")

            # Apply filters if using Urban Vyapari integration
            if filters['status'] != 'all':
                result = [t for t in result if t['status']
                          == filters['status']]
            if filters['priority'] != 'all':
                result = [t for t in result if t['priority']
                          == filters['priority']]
            if filters['category'] != 'all' and filters['category'] != 'Unknown':
                result = [t for t in result if t['category'].lower()
            ...
            if hasattr(request, 'uv_admin'):
                ...
                logger.info(
                    f"Urban Vyapari access by: {request.uv_admin['admin_name']}")

            return jsonify(response_data)

        except Exception as query_error:
            logger.error(
                f"Error querying tickets with joins: {str(query_error)}", exc_info=True)

            # Try a simpler query as fallback
            try:
                simple_tickets = Ticket.query.all()
                logger.info(
                    f"Fallback: Found {len(simple_tickets)} tickets with simple query")
        ...
        logger.error(
            f"Unexpected error in get_admin_tickets: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
...
def get_admin_ticket_details(ticket_id):
    try:
        ...
        for msg in messages:
            # Get attachments for this message
            attachments = Attachment.query.filter_by(
                MessageID=msg.MessageID).all()
            logger.info(
                f"Message {msg.MessageID} has {len(attachments)} attachments")
            ...
            if attachments:
                ...
                logger.info(
                    f"Added attachment URL: {message_data['attachment_url']}")
        ...
        result = {
            ...
            'country': ticket_obj.Country if ticket_obj.Country else 'Unknown',
            'messages': message_list
        }

        logger.info(
            f"Returning ticket details for {ticket_id} with {len(message_list)} messages")
        return jsonify(result)
    except Exception as e:
        logger.error(
            f"Error getting admin ticket details for {ticket_id}: {str(e)}", exc_info=True)
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        for ticket, user, category in conversations:
            ...
            last_message = Message.query.filter_by(
                TicketID=ticket.TicketID).order_by(Message.CreatedAt.desc()).first()

            # Count unread admin messages (assuming we track this)
            unread_count = Message.query.filter_by(
                TicketID=ticket.TicketID, IsAdminReply=False).count()
...
def submit_feedback():
    ...
    try:
        ...
        logger.info(
            f"Feedback submitted for ticket {ticket_id}: Rating={rating}")
...
def health_check_container():
    ...
    try:
        ...
        return jsonify({
            ...
            'version': '1.0.0',
            'database': 'connected',
            'odoo': 'available' if odoo_service else 'unavailable'
        }), 200
...
# Import Bot Service and Enhanced SLA Monitor
...
try:
    ...
    with app.app_context():
        ...
        if table_count >= 3:
            enhanced_sla_monitor.start_monitoring()
            logger.info("Enhanced SLA monitoring service started successfully")
        else:
            logger.info(
                "SLA monitoring service not started - tables not ready")
except Exception as e:
...
try:
    ...
    def generate_uv_access_token():
        ...
        try:
            ...
            logger.info(
                f"Generated Urban Vyapari token for admin: {admin_info['admin_name']} ({admin_info['admin_email']})")
    ...
    def tickets_portal_uv():
        ...
        try:
            ...
            logger.info(
                f"Urban Vyapari admin accessing tickets portal: {payload['admin_name']}")
