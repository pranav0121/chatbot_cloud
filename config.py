...
class Config:
    ...
    DB_USE_WINDOWS_AUTH = os.getenv(
        'DB_USE_WINDOWS_AUTH', 'True').lower() in ('true', '1', 't')
    ...
    def SQLALCHEMY_DATABASE_URI(self):
        ...
        if self.DB_USE_WINDOWS_AUTH:
            # Use Windows Authentication with simplified connection string
            ...
            # Use SQL Server Authentication with simplified connection string
            driver = quote_plus(drivers_to_try[0])
            password = quote_plus(self.DB_PASSWORD) if self.DB_PASSWORD else ''

            # Handle Docker host.docker.internal format
            if 'host.docker.internal' in self.DB_SERVER:
                server = self.DB_SERVER.replace(
                    '\\', '%5C')  # URL encode backslash
            else:
                server = self.DB_SERVER

            return f'mssql+pyodbc://{self.DB_USERNAME}:{password}@{server}/{self.DB_DATABASE}?driver={driver}'
