...
async function loadChatMessages() {
    ...
    try {
        ...
        if (ticketStatus && (ticketStatus === 'resolved' || ticketStatus === 'closed')) {
            // Only handle closure once to prevent repeated disabling
            if (chatState !== 'closed') {
...
async function sendMessage() {
    ...
    if (chatState === 'closed') {
        showNotification('This conversation has been closed.', 'info');
        return;
    }
...
async function sendMessageWithAttachment(content, isAdmin = false, filename = null) {
    if (!currentTicketId) {
        showNotification('Please start a new ticket first.', 'error');
        return;
    }
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            addChatMessage(content, isAdmin ? 'admin' : 'user', new Date().toISOString());
            
            // Clear input if this was a manual message
            if (!filename) {
                const input = document.getElementById('live-chat-message-input') || document.getElementById('message-input');
...
// Paste functionality for images and documents
...
async function uploadTempFile(file, source = 'upload') {
    if (!file) return;
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            const message = source === 'paste' ? 
                `📎 Pasted file: ${file.name}` : 
                `📎 Attached file: ${file.name}`;
            
            await sendMessageWithAttachment(message, false, data.filename);
        } else {
...
// Enhanced sendMessage function to prevent disabling
async function sendMessage() {
    // Try to get message from either input field
    const messageInput = document.getElementById('message-input');
    const liveChatInput = document.getElementById('live-chat-message-input');
