...
def get_partners():
    ...
    try:
        ...
        for partner in partners:
            ...
            sla_compliance = 100 - \
                (breached_slas / recent_tickets *
                 100) if recent_tickets > 0 else 100
...
def update_partner(partner_id):
    ...
    try:
        ...
        partner.contact_person = data.get(
            'contact_person', partner.contact_person)
        ...
        if 'escalation_settings' in data:
            partner.escalation_settings = json.dumps(
                data['escalation_settings'])
...
def delete_partner(partner_id):
    ...
    try:
        ...
        log_admin_action('delete', 'partner', partner_id,
                         {'name': partner_name})
    ...
    try:
        ...
        for ticket in tickets:
            try:
                ...
                hours_passed = (
                    current_time - ticket.CreatedAt).total_seconds() / 3600
                ...
                logger.error(
                    f"Error processing ticket {ticket.TicketID}: {ticket_error}")
                # Still count as within SLA if we can't process it
                within_sla += 1

        logger.info(
            f"SLA stats - Within: {within_sla}, Warning: {sla_warning}, Breached: {sla_breached}")
...
def force_escalate_ticket(ticket_id):
    ...
    try:
        ...
        logger.info(
            f"Escalation parameters: level={new_level}, comment={comment}")

        ticket = Ticket.query.get_or_404(ticket_id)
        logger.info(
            f"Found ticket: {ticket.Subject}, current status: {ticket.Status}")
        ...
        if available_partners:
            ...
            logger.info(
                f"Assigned ticket to partner: {assigned_partner.name} (ID: {assigned_partner.id})")

            # Update partner statistics
            assigned_partner.total_tickets_handled = (
                assigned_partner.total_tickets_handled or 0) + 1
        ...
        logger.info(
            f"Created SLA log: level={new_level}, partner_id={assigned_partner.id if assigned_partner else None}")
        ...
        logger.info(
            f"BEFORE UPDATE: status={old_status}, level={old_escalation_level}, partner={old_partner_id}")
        ...
        logger.info(
            f"AFTER UPDATE (before commit): status={ticket.Status}, level={ticket.escalation_level}, partner={ticket.partner_id}")
        ...
        try:
            ...
            logger.info(
                f"AFTER COMMIT (verified): status={ticket.Status}, level={ticket.escalation_level}, partner={ticket.partner_id}")
        ...
        if assigned_partner and assigned_partner.webhook_url:
            try:
                ...
                logger.info(
                    f"Webhook notification sent to {assigned_partner.name}")
...
def search_logs():
    ...
    try:
        ...
        if escalation_level:
            query = query.filter(
                TicketStatusLog.escalation_level == escalation_level)
...
def get_audit_logs():
    ...
    try:
        ...
        try:
            ...
            # Execute query with pagination
            logs = query.order_by(AuditLog.created_at.desc()).limit(
    ...
    try:
        ...
        if config:
            config_data = json.loads(
                config.config_data) if config.config_data else {}
            return jsonify({
                'success': True,
                'config': {
    ...
    try:
        ...
        if config:
            ...
            config.confidence_threshold = data.get(
                'confidence_threshold', config.confidence_threshold)
            config.fallback_to_human = data.get(
                'fallback_to_human', config.fallback_to_human)
            config.is_active = data.get('enabled', config.is_active)
            config.config_data = json.dumps({k: v for k, v in data.items() if k not in [
                                            'enabled', 'bot_type', 'confidence_threshold', 'fallback_to_human']})
            ...
            config = BotConfiguration(
                ...
                confidence_threshold=data.get('confidence_threshold', 0.7),
                fallback_to_human=data.get('fallback_to_human', True),
                config_data=json.dumps({k: v for k, v in data.items() if k not in [
                                       'enabled', 'bot_type', 'confidence_threshold', 'fallback_to_human']})
            )
            db.session.add(config)
...
def get_bot_status():
    ...
    try:
        ...
        accuracy = (successful_interactions / total_interactions *
                    100) if total_interactions > 0 else 0

        # Calculate average response time
        response_times = [
            i.response_time for i in recent_interactions if i.response_time]
...
def get_ticket_details(ticket_id):
    ...
    try:
        ...
        ticket = db.session.query(Ticket).filter(
            Ticket.TicketID == ticket_id).first()
...
def get_sla_overview_legacy():
    ...
    try:
        ...
        for priority, target_hours in sla_targets.items():
            priority_tickets = [
                t for t in active_tickets if t.Priority.lower() == priority]
            ...
            compliance_percentage = round(
                (total_count - breached_count) / total_count * 100, 1)
...
def get_sla_overview():
    ...
    try:
        ...
        for priority, target_hours in sla_targets.items():
            ...
            compliance_rate = (within_sla / total_tickets *
                               100) if total_tickets > 0 else 100
...
def get_sla_analytics():
    ...
    try:
        ...
        for i in range(days):
            ...
            if day_tickets:
                sla_targets = {'critical': 2,
                               'high': 4, 'medium': 24, 'low': 48}
                compliant = 0
                ...
                for ticket in day_tickets:
                    ...
                    if ticket.Status == 'resolved':
                        ...
                        hours_passed = (
                            end_date - ticket.CreatedAt).total_seconds() / 3600
                        if hours_passed <= target_hours:
                            compliant += 1
...
def get_dashboard_metrics_fixed():
    ...
    try:
        ...
        total_tickets = db.session.query(
            func.count(Ticket.TicketID)).scalar() or 0
...
def debug_database():
    ...
    try:
        ...
        # Test basic counts
        results['total_tickets'] = db.session.query(
            func.count(Ticket.TicketID)).scalar()
        results['total_users'] = db.session.query(
            func.count(User.UserID)).scalar()
        results['total_messages'] = db.session.query(
            func.count(Message.MessageID)).scalar()

        # Test status distribution
        statuses = db.session.query(Ticket.Status, func.count(
            Ticket.TicketID)).group_by(Ticket.Status).all()
